...
void CPhrasesGenerator::GeneratePhrasesFromParsedFiles()
{
    ...
    {
        std::string lang_utf8 = U16StringToUTF8(language);
        std::string languageCode = m_LanguageCodeNames[lang_utf8];

        if (languageCode.empty())
        {
            std::cout << "Language \"" << lang_utf8 << "\" has no code" << std::endl;
            continue;
        }
        ...
        std::ofstream phrasesFile(
            translationsBasePath / 
                (lang_utf8 != "english" ? languageCode : "") /
                    "localization.phrases.txt"
        );

        if (phrasesFile.fail())
        {
            std::cout << "Failed to open " << lang_utf8 << " phrases file." << std::endl;
            continue;
        }

        // Write phrases file header.
        phrasesFile << "// " << lang_utf8 << " phrases file" "\n"
        ...
        std::cout << "Generated " << langTokens.size() << " " << lang_utf8 << " phrases." << std::endl;
    }
}
