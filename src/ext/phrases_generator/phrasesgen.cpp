...
void CPhrasesGenerator::GeneratePhrasesFromParsedFiles()
{
    ...
    {
        std::string langUTF8 = U16StringToUTF8(language);
        std::string languageCode = m_LanguageCodeNames[langUTF8];

        if (languageCode.empty())
        {
            std::cout << "Language \"" << langUTF8 << "\" has no code" << std::endl;
            continue;
        }
        ...
        std::ofstream phrasesFile(
            translationsBasePath / 
                (langUTF8 != "english" ? languageCode : "") /
                    "localization.phrases.txt"
        );

        if (phrasesFile.fail())
        {
            std::cout << "Failed to open " << langUTF8 << " phrases file." << std::endl;
            continue;
        }

        // Write phrases file header.
        phrasesFile << "// " << langUTF8 << " phrases file" "\n"
        ...
        std::cout << "Generated " << langTokens.size() << " " << langUTF8 << " phrases." << std::endl;
    }
}
